...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.insertId, name: name.trim(), file_count: 0 });
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Folder name already exists' });
